...
- When the user changes the input of the drop-down, update the background image based on what they selected (done)
- Use the ```$.attr()``` function to update html classes (DONE)
- Get the value of user input using ```$.val()``` (DONE)
- Use ```if/else if/else ``` conditionals to control the flow of your application (DONE)
